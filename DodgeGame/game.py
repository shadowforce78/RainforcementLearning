...
class AI:
    def __init__(self):
        # Charger la Q-table si elle existe, sinon en créer une nouvelle
        if os.path.exists(Q_TABLE_FILE):
            self.q_table = np.load(Q_TABLE_FILE)
            print("Q-table chargée depuis le fichier")
        else:
            self.q_table = np.zeros((WIDTH // PLAYER_SIZE, 2))
            print("Nouvelle Q-table créée")
            
        self.epsilon = 0.3  # Exploration de départ plus faible après plusieurs épisodes
...
def draw_ai_visualization(screen, ai, player, obstacles, current_state, action):
    ...
    for x in range(num_cells):
        ...
        pygame.draw.rect(overlay, left_color, (x * cell_width + 5, HEIGHT - left_height - 110, 15, left_height))
        pygame.draw.rect(overlay, right_color, (x * cell_width + 30, HEIGHT - right_height - 110, 15, right_height))
        
        # Indiquer l'action optimale pour cet état
        best_action = np.argmax(ai.q_table[state])
        if best_action == 0:  # Gauche
            pygame.draw.polygon(overlay, YELLOW, [(x * cell_width + 12, HEIGHT - 120), 
                                               (x * cell_width + 2, HEIGHT - 130),
                                               (x * cell_width + 22, HEIGHT - 130)])
        else:  # Droite
            pygame.draw.polygon(overlay, YELLOW, [(x * cell_width + 38, HEIGHT - 120), 
    ...
    pygame.draw.rect(overlay, CYAN, (predicted_x, player.y, PLAYER_SIZE, PLAYER_SIZE), 2)
    ...
    for obstacle in obstacles:
        ...
        pygame.draw.rect(overlay, (255, 0, 0, 30), 
    ...
    pygame.draw.polygon(overlay, YELLOW, [(WIDTH - 156, 98), (WIDTH - 160, 108), (WIDTH - 152, 108)])
...
def game_episode(ai, episode_num, training_speed):
    ...
    while running:
        ...
        for obstacle in obstacles[:]:
            ...
            if (
                obstacle.x < player.x < obstacle.x + OBSTACLE_SIZE
                or obstacle.x < player.x + PLAYER_SIZE < obstacle.x + OBSTACLE_SIZE
        ...
        next_state = ai.get_state(player)
        ...
        font = pygame.font.SysFont(None, 24)
        episode_text = font.render(f"Episode: {episode_num}/{MAX_EPISODES}", True, WHITE)
        ...
        ai_view_text = font.render(f"Vue IA: {'ON' if show_ai_view else 'OFF'} (V)", 
                                 True, PURPLE if show_ai_view else WHITE)
...
def main():
    ...
    while episode < MAX_EPISODES:
        ...
        if episode % 10 == 0:
            ai.epsilon = max(0.01, ai.epsilon * 0.95)
